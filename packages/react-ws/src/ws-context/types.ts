...
export interface CreateWsContextOptions {
  /**
   * WebSocket URL。可為字串或同步 getter。
   *
   * 每次建立 socket 前同步呼叫；不要 `await`、不要在裡頭用 hook。
   */
   ...
   * 每重連一次，下次要等的時間放大幾倍。
   *
   * 預設 `2`：`reconnectMs: 1000` 時依序等 1、2、4、8 秒。設 `1` 為固定間隔；小於 `1` 會夾回 `1`。
   *
   * 成長天花板見 `reconnectDelayMaxMs`。
   *
   * @default 2
   */
  reconnectBackoff?: number;
  /**
   * 單次重連等待上限（毫秒）；抖動後也不會超過。
   *
   ...
   *
   * 預設 `0.2`：實際等預定時間的 80%–100%，避免 server 重啟後所有 client 同時湧入。
   *
   * `1` 為 full jitter（0 到預定時間）；`0` 則每次等足預定時間。
   *
   * @default 0.2
   */
  reconnectJitter?: number;
  /**
   * 連線要撐多久（毫秒）才把 `reconnectAttempt`／`reconnectExhausted` 歸零。
   *
   * 預設 5 秒是為了應付 server 收下連線後立刻又斷開：設 `0` 會讓每個短命連線都清掉
...
export interface WsContextValue {
  ...
  /**
   * 以 JSON 傳送。無法序列化時回傳 `false`。
   *
   * @returns 同 `send`
   */
  sendJson: (data: unknown) => boolean;
  /**
   * 建立連線。每次重新取值（`url`／`protocols`），先建新 socket，成功後才關閉舊連線。
   *
   * 取值失敗、網址為空或不合法時發出 `"error"`，既有連線不受影響；`connect()` 本身不 throw。
   *
   * 若自動重連計時器已觸發後才失敗：進入 `status: "closed"`、`phase: "stopped"`，不再自動重試。
   */
  connect: () => void;
  /** 主動斷線；不觸發自動重連，清空待送佇列 */
  disconnect: () => void;
